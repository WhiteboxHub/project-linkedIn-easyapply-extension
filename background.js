...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.action === 'startApply') {
    ...
    const candidate_id = msg.candidate_id;
    const employee_id = msg.employee_id;
    if (typeof candidate_id === 'undefined' || typeof employee_id === 'undefined' || !candidate_id || !employee_id) {
      sendResponse({ message: 'Missing selection' }); return;
    }

    // initialize run meta and log
    ...
    chrome.storage.local.set({
      ...
      (async () => {
        try {
          await startApplyFlow();
          // note: startApplyFlow will call finalizeRunAndExport when finished
        } catch (e) {
          console.error('BG: startApplyFlow error', e);
          chrome.runtime.sendMessage({ from: 'background', type: 'error', text: String(e) });
          try { await finalizeRunAndExport(); } catch (ee) { console.error('BG: finalize after error failed', ee); }
        } finally {
          running = false;
        }
      })();
  ...
  // Stop run - will finalize current collected submissions
  if (msg.action === 'stopApply') {
    // toggles running flag off; finalizeRunAndExport will be called here to make sure we export whatever we've collected so far
    (async () => {
      if (!running) {
        // if not running but meta exists, still finalize
        try {
          await finalizeRunAndExport();
          sendResponse && sendResponse({ message: 'Stopped (no active run)' });
        } catch (e) {
          console.error('BG: stop finalize error', e);
          sendResponse && sendResponse({ message: 'Stopped with error during finalize' });
        }
        return;
      }
      running = false;
      // allow the main loop to finish current job; then call finalize
      try {
        await finalizeRunAndExport();
        sendResponse && sendResponse({ message: 'Stopping and exported run' });
      } catch (e) {
        console.error('BG: stopFinalize error', e);
        sendResponse && sendResponse({ message: 'Stopping; finalize failed: ' + String(e) });
      }
    })();
  ...
  // Debug: load jobs from easyapply_today.json
  if (msg.action === 'debugLoadJobs') {
    (async () => {
      try {
        const data = await loadJobs();
        ...
        sendResponse({ message: 'Failed to load jobs: ' + String(e) });
      }
    })();
    return true;
  }

  // Received from content_script when an application is successfully submitted
  if (msg.type === 'application_submitted') {
    // append to current run log
    chrome.storage.local.get([STORAGE.CURRENT_RUN_META, STORAGE.CURRENT_RUN_LOG, STORAGE.CURRENT_SELECTION], (data) => {
      const runLog = Array.isArray(data[STORAGE.CURRENT_RUN_LOG]) ? data[STORAGE.CURRENT_RUN_LOG] : [];
      const sel = data[STORAGE.CURRENT_SELECTION] || {};
      const entry = {
        candidate_id: Number(msg.candidate_id || sel.candidate_id || 0),
        employee_id: Number(msg.employee_id || sel.employee_id || 0),
        jobInfo: msg.jobInfo || {},
        timestamp: msg.timestamp || new Date().toISOString()
      };
      runLog.push(entry);
      chrome.storage.local.set({ [STORAGE.CURRENT_RUN_LOG]: runLog }, () => {
        const short = `Recorded submission for candidate ${entry.candidate_id} (job ${entry.jobInfo.jobId})`;
        chrome.runtime.sendMessage({ from: 'background', type: 'progress', text: short });
...
async function startApplyFlow() {
  ...
  while (running && currentIndex < jobs.length) {
    ...
    const jobUrl = `https://www.linkedin.com/jobs/view/${job.jobId}`;
    let tab;
    try { tab = await createTab(jobUrl); } catch (e) { console.error('BG: createTab failed', e); currentIndex++; continue; }
    ...
    try {
      ...
      if (resp && resp.result && resp.result.applied) {
        // successful application recorded by content script (which also sent a message to store the submission)
        await delay(POST_APPLY_WAIT_MS);
      } else {
        await delay(1000);
      }
  ...
  return new Promise((resolve, reject) => {
    chrome.storage.local.get([STORAGE.CURRENT_RUN_META, STORAGE.CURRENT_RUN_LOG], async (data) => {
      try {
        ...
        const candidateObj = (cfg.candidates||[]).find(c=>c.id===sel.candidate_id) || { id: sel.candidate_id, name: String(sel.candidate_id || 'unknown') };
        const employeeObj = (cfg.employees||[]).find(e=>e.id===sel.employee_id) || { id: sel.employee_id, name: String(sel.employee_id || 'unknown') };
        ...
        // Download single JSON file for this run
        const jsonStr = JSON.stringify(exportObj, null, 2);
        const filename = `easyapply_run_${meta.runId}_${candidateObj.id}_${new Date().toISOString().slice(0,19).replace(/[:T]/g,'-')}.json`;
        const url = 'data:application/json;charset=utf-8,' + encodeURIComponent(jsonStr);

        chrome.downloads.download({ url, filename, conflictAction: 'uniquify', saveAs: false }, async (downloadId) => {
          if (chrome.runtime.lastError) console.error('BG: finalize download error', chrome.runtime.lastError);
          else console.log('BG: downloaded run file', filename, downloadId);

          // Aggregate rows by candidate/employee/date for server
          const map = {};
          for (const e of runLog || []) {
            const cid = Number(e.candidate_id || 0);
            const eid = Number(e.employee_id || 0);
            // date in YYYY-MM-DD
            const activity_date = (new Date(e.timestamp)).toISOString().slice(0,10);
            const key = `${cid}|${eid}|${activity_date}`;
            if (!map[key]) map[key] = { candidate_id: cid, employee_id: eid, activity_date, activity_count: 0, job_ids: [] };
            map[key].activity_count += 1;
            if (e.jobInfo && e.jobInfo.jobId) map[key].job_ids.push(String(e.jobInfo.jobId));
          }
          const rows = Object.values(map);

          // Load server API config
          let apiCfg = null;
          try { apiCfg = await fetch(chrome.runtime.getURL('config/api_config.json')).then(r=>r.json()); } catch (e) { apiCfg = null; }

          if (apiCfg && apiCfg.api_url) {
            const payload = { runId: meta.runId, startedAt: meta.startedAt, exportedAt: new Date().toISOString(), default_job_id: apiCfg.default_job_id || null, rows };
            try {
              const resp = await fetch(apiCfg.api_url, {
                method: 'POST',
              ...
              if (!resp.ok) {
                ...
                console.error('BG: server returned error', resp.status, text);
              } else {
                try {
                  const j = await resp.json().catch(()=>null);
                  console.log('BG: server accepted summary', j);
                } catch(e) {}
              }
            } catch (e) {
              console.error('BG: failed to send run summary to server', e);
            }
          } else {
            console.warn('BG: no api_url configured; skipping server POST');
          }

          // clear stored run meta and log (but keep currentSelection for UX)
          chrome.storage.local.remove([STORAGE.CURRENT_RUN_META, STORAGE.CURRENT_RUN_LOG], () => {
            console.log('BG: cleared current run meta & log');
            // notify popup (if open)
            chrome.runtime.sendMessage({ from: 'background', type: 'done', text: 'Run exported' });
            resolve(downloadId);
          });
        });

      } catch (e) {
        console.error('BG: finalizeRunAndExport error', e);
        reject(e);
      }
    });
  });
